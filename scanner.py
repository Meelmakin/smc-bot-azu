...
def find_signal(m15, levels, sides=("long", "short"), a=None):
...
def scan_pair(name, ticker):
    ...
    if TREND_ON:
        ...
        sides = ("long",) if h1["close"].iloc[-1] > ema.iloc[-1] else ("short",)
    trend = "up" if sides == ("long",) else ("down" if TREND_ON else "off")
    return find_signal(m15, levels, sides, a), f"{len(levels)} levels, 1H trend {trend}"
